...
///
/// 预算只约束可以重新计算的加速项，不得把光源注册表、已发布光场等真实状态放进本缓存。
/// 预算不足时只淘汰最久未访问的加速项，正确结果必须能够由真实状态重新构建。
///
/// 缓存取得每个映射值的所有权。映射因替换、显式删除、条件删除、清空或预算淘汰而离开
/// 缓存时，都会恰好调用一次 disposer。同一 key 再次放入同一个对象引用视为更新该映射的
/// 权重，所有权没有离开缓存，因此更新时不调用 disposer；若更新后的映射又被预算淘汰，
/// 则仍会调用一次。不要把同一个无租约资源同时放在多个 key 下，因为每个映射都独立拥有值。
