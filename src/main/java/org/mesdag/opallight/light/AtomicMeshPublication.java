...
///
/// staging 在 commit 之前完全不可见；commit 只交换 map 引用并把旧 active 作为
/// [OwnedState] 转交给调用方。调用方可以把旧状态放入 generation cache，也可以立即
/// 关闭。构建、上传或取消路径只要关闭 stage，就只会释放 staging，不会修改 active。
///
